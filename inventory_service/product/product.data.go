...
func searchForProductData(productFilter ProductReportFilter) ([]Product, error) {
	...
	result, err := database.DBConnect.QueryContext(ctx, queryBuilder.String(), queryArgs...)
	if err != nil {
...
func GetTopTenProducts() ([]Product, error) {
	...
	defer cancel()
	result, err := database.DBConnect.QueryContext(ctx, `SELECT productId,
																manufacturer,
																sku,
																upc,
																pricePerUnit,
...
func removeProduct(productID int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	_, err := database.DBConnect.QueryContext(ctx, `DELETE FROM products WHERE productId = ?`, productID)
	return err
}
...
func insertProduct(product Product) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	result, err := database.DBConnect.ExecContext(ctx, `INSERT INTO products
...
func updateProduct(product Product) error {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	_, err := database.DBConnect.ExecContext(ctx, `UPDATE products SET 
			manufacturer = ?,
			sku = ?,
			upc = ?,
...
func getProduct(productID int) (*Product, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	row := database.DBConnect.QueryRowContext(ctx, `SELECT productId,
	 manufacturer,
	 sku,
	 upc,
	 pricePerUnit,
...
func getProductList() ([]Product, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	results, err := database.DBConnect.QueryContext(ctx, `SELECT productId,
	 manufacturer,
	 sku,
	 upc,
	 pricePerUnit,
